...
class SingleProcessSpider(scrapy.Spider):
    ...
    def start_requests(self):
        start = 0
        while True:
            movie_list_page_url = f'https://movie.douban.com/j/chart/top_list?type=17&interval_id=100%3A90&action=&start={start}&limit=20'
            proxy = requests.get('http://139.9.128.8:5555/random').text
            proxies = {
                "http": "http://" + proxy,
                "https": "https://" + proxy
            }
            ...
            result = requests.get(movie_list_page_url, headers=headers, proxies=proxies)
